...
export default function AjouterEnleverPanier({ product, stock, onQuantityChange, quantite }) {
  ...
  return (
    <div className={styles.achatWrapper}>
      ...
      <div><DecrementerProduitCarte product={product} onQuantityChange={onQuantityChange} quantite={quantite} addToCart={addToCart} /></div>
      <div className={styles.panierItemQuantite}>{quantite}</div>
      <IncrementerProduitCarte product={product} onQuantityChange={onQuantityChange} quantite={quantite} addToCart={addToCart} stock={stock} />
    </div>
  );
}
